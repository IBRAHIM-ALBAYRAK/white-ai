...
# ── Merkez (sube-siz, dogrudan markaya bagli personel) + Franchise toplamlari ──
from app.modules.company.models import Company, OversightLink
from app.modules.payroll.schemas import RunMerkezSchema, FranchiseTotalSchema

owner_only = require_role(UserRole.SUPERADMIN, UserRole.OWNER)


@router.get("/merkez/{year}/{month}", response_model=list[EmployeePayrollRowSchema])
async def get_merkez_month(
    year: int, month: int,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(owner_only),
):
    """Markanin sube-siz (branch_id IS NULL) kendi personeli. Owner tam yetki."""
    emps = (await db.execute(
...
@router.post("/run-merkez", response_model=list[RunBranchResultSchema])
async def run_merkez(
    data: RunMerkezSchema,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(owner_only),
):
    """Merkez (sube-siz) personelin donem bordrosunu calistir. Owner-only."""
    emps = (await db.execute(
        select(Employee).where(
            Employee.company_id == current_user.company_id,
            Employee.branch_id.is_(None),
...
@router.get("/franchise-totals/{year}/{month}", response_model=list[FranchiseTotalSchema])
async def franchise_totals(
    year: int, month: int,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(owner_only),
):
    """Franchise sirketlerinin SADECE toplam isveren maliyeti. Calisan-bazli veri donmez (KVKK)."""
    links = (await db.execute(
        select(OversightLink).where(
            OversightLink.brand_company_id == current_user.company_id,
            OversightLink.link_type == "franchise",
        )
    )).scalars().all()
    out: list[FranchiseTotalSchema] = []
    for link in links:
        comp = (await db.execute(
            select(Company).where(Company.id == link.sub_company_id)
        )).scalar_one_or_none()
        if comp is None:
            continue
        emps = (await db.execute(
            select(Employee).where(Employee.company_id == comp.id, Employee.is_active == True)
        )).scalars().all()
        ...
        if emp_ids:
            slips = (await db.execute(
                select(Payslip).where(
                    Payslip.employee_id.in_(emp_ids), Payslip.year == year, Payslip.month == month
                )
            )).scalars().all()
            calc = len(slips)
            total = float(sum(float(p.employer_cost) for p in slips))
        out.append(FranchiseTotalSchema(
            company_id=comp.id, name=comp.name,
            employee_count=len(emps), calculated_count=calc, total_employer_cost=total,
        ))
    return out
