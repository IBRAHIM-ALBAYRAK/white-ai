...
export default function BrandPanel({
  ...
  const [financeTab, setFinanceTab] = useState<"overview" | "branches" | "franchise">("overview");
  const [payrollTab, setPayrollTab] = useState<"merkez" | "branches" | "franchise">("merkez");
  const [staffTab, setStaffTab] = useState<"merkez" | "branches" | "franchise">("merkez");
      ...
      <main style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", background: C.bg }}>
        <div style={{ padding: "16px 26px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            {page === "finance" && (
              <div style={{ display: "inline-flex", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 11, padding: 3 }}>
                {(["overview", "branches", "franchise"] as const).map((t) => (
                  <div key={t} onClick={() => setFinanceTab(t)} style={{ padding: "6px 14px", fontSize: 12.5, fontWeight: financeTab === t ? 600 : 500, color: financeTab === t ? "#fff" : C.textFaint, background: financeTab === t ? C.ink : "transparent", borderRadius: 8, cursor: "pointer" }}>{t === "overview" ? "Genel Bakış" : t === "branches" ? "Şubeler" : "Franchise"}</div>
                ))}
              </div>
            )}
            {page === "payroll" && (
              <div style={{ display: "inline-flex", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 11, padding: 3 }}>
                {(["merkez", "branches", "franchise"] as const).map((t) => (
                  <div key={t} onClick={() => setPayrollTab(t)} style={{ padding: "6px 14px", fontSize: 12.5, fontWeight: payrollTab === t ? 600 : 500, color: payrollTab === t ? "#fff" : C.textFaint, background: payrollTab === t ? C.ink : "transparent", borderRadius: 8, cursor: "pointer" }}>{t === "merkez" ? "Marka" : t === "branches" ? "Şubeler" : "Franchise"}</div>
                ))}
              </div>
            )}
            {page === "staff" && (
              <div style={{ display: "inline-flex", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 11, padding: 3 }}>
                {(["merkez", "branches", "franchise"] as const).map((t) => (
                  <div key={t} onClick={() => setStaffTab(t)} style={{ padding: "6px 14px", fontSize: 12.5, fontWeight: staffTab === t ? 600 : 500, color: staffTab === t ? "#fff" : C.textFaint, background: staffTab === t ? C.ink : "transparent", borderRadius: 8, cursor: "pointer" }}>{t === "merkez" ? "Marka" : t === "branches" ? "Şubeler" : "Franchise"}</div>
                ))}
              </div>
            )}
        ...
        <div style={{ padding: "24px 26px", overflow: "auto" }}>
          ...
          {page === "staff" && <StaffPage token={token} companyId={user.company_id || ""} jumpBranch={jumpBranch} clearJump={() => setJumpBranch(null)} tab={staffTab} setTab={setStaffTab} />}
          {page === "inventory" && <InventoryPage token={token} companyId={user.company_id || ""} jumpBranch={jumpBranch} clearJump={() => setJumpBranch(null)} />}
          {page === "payroll" && <PayrollPage token={token} companyId={user.company_id || ""} jumpBranch={jumpBranch} clearJump={() => setJumpBranch(null)} tab={payrollTab} setTab={setPayrollTab} />}
          {page === "ledger" && <LedgerPage token={token} companyId={user.company_id || ""} />}
          {page === "finance" && <FinancePage token={token} companyId={user.company_id || ""} tab={financeTab} setTab={setFinanceTab} />}
...
// ── Vergi motoru (2026, ManagerFinans'tan porte) ──────────────────────────
const KURUMLAR: Record<string, number> = {
  yeme_icme: 0.25, ticaret: 0.25, hizmet: 0.25, insaat: 0.25,
  bilisim: 0.25, diger: 0.25, uretim: 0.125, tarim: 0.125, finans: 0.30,
};
const STOPAJ_F = 0.15;
const SAHIS_BR = [
  { limit: 190000, rate: 0.15 }, { limit: 400000, rate: 0.20 },
  { limit: 1000000, rate: 0.27 }, { limit: 5300000, rate: 0.35 },
  { limit: Infinity, rate: 0.40 },
];
function computeFinTax(legalType: string, sector: string, monthlyProfit: number): number {
  if (monthlyProfit <= 0) return 0;
  if (legalType !== "sahis") {
    const k = KURUMLAR[sector] ?? 0.25;
    const kurumlar = monthlyProfit * k;
    const stopaj = (monthlyProfit - kurumlar) * STOPAJ_F;
    return kurumlar + stopaj;
  }
  const annual = monthlyProfit * 12;
  let tax = 0, prev = 0;
  for (const b of SAHIS_BR) {
...
const LEGAL_LABELS: Record<string, string> = { limited: "Limited Şti.", anonim: "Anonim Şti.", sahis: "Şahıs", kurumsal: "Kurumsal" };
const SECTOR_LABELS: Record<string, string> = { yeme_icme: "Yeme-İçme", ticaret: "Ticaret", hizmet: "Hizmet", insaat: "İnşaat", bilisim: "Bilişim", uretim: "Üretim", tarim: "Tarım", finans: "Finans", diger: "Diğer" };

// ── Finans — Genel Bakış (marka geneli, salt-okunur) ──────────────────────
function FinancePage({ token, companyId, tab, setTab }: { token: string; companyId: string; tab: "overview" | "branches" | "franchise"; setTab: (t: "overview" | "branches" | "franchise") => void }) {
  ...
  useEffect(() => { if (tab !== "branches") setSelBranch(null); }, [tab]);

  const load = async () => {
    setLoading(true);
    try { const r = await axios.get(`${API_URL}/finance/overview/${year}/${month}?t=${Date.now()}`, { headers }); setData(r.data); }
    catch { setData(null); }
    setLoading(false);
  };
  useEffect(() => { load(); /* eslint-disable-next-line */ }, [year, month, companyId]);
  useEffect(() => {
    axios.get(`${API_URL}/finance/tax-profile`, { headers }).then((r) => { if (r.data?.legal_type) setLegalType(r.data.legal_type); if (r.data?.sector) setSector(r.data.sector); }).catch(() => {});
    axios.get(`${API_URL}/companies/${companyId}/branches?t=${Date.now()}`, { headers }).then((r) => setBranches(r.data || [])).catch(() => {});
    /* eslint-disable-next-line */
  }, [companyId]);

  const prevMonth = () => { if (month === 1) { setMonth(12); setYear(year - 1); } else setMonth(month - 1); };
  const nextMonth = () => { const atCur = year === today.getFullYear() && month === today.getMonth() + 1; if (atCur) return; if (month === 12) { setMonth(1); setYear(year + 1); } else setMonth(month + 1); };
  const atCurrent = year === today.getFullYear() && month === today.getMonth() + 1;

  const income = data?.manual_income ?? 0;
  const expense = data?.manual_expense ?? 0;
  const preTax = income - expense;
  const tax = computeFinTax(legalType, sector, preTax);
  const totalExpense = expense + tax;
  const net = income - totalExpense;
  const margin = income > 0 ? (net / income) * 100 : 0;
  const incomeCats = data?.income_categories ?? [];
  const expenseCats = data?.expense_categories ?? [];
  const isEmpty = !loading && income === 0 && expense === 0;
  const donut = incomeCats.map((c, i) => ({ name: c.name, value: c.total, color: GELIR_RENK[i % GELIR_RENK.length] }));
  const barMax = Math.max(income, totalExpense, tax, Math.abs(net), 1);
  const bars = [{ label: "Gelir", val: income, color: "#1FA85A" }, { label: "Giderler", val: expense, color: "#C0564B" }, { label: "Devlet/Vergi", val: tax, color: "#7C6B9A" }, { label: "Net", val: net, color: net >= 0 ? "#1FA85A" : "#C0564B" }];

  const card: React.CSSProperties = { background: C.bg, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, boxShadow: "0 1px 3px rgba(0,0,0,0.03)" };

  return (
    <div>
      {tab === "franchise" ? (
        <div style={{ ...card, textAlign: "center", padding: "60px 20px" }}>
          <i className="ti ti-tools" style={{ fontSize: 30, color: C.textHint }} aria-hidden="true" />
          <div style={{ fontSize: 14, fontWeight: 600, color: C.ink, marginTop: 12 }}>Franchise bazlı finans — yakında</div>
          <div style={{ fontSize: 12.5, color: C.textFaint, marginTop: 4 }}>Her franchise'ın markaya katkısı ve kendi finansı burada açılacak.</div>
        </div>
      ) : tab === "branches" ? (
        selBranch ? (
          <BranchFinanceDetail token={token} branchId={selBranch} branchName={branches.find((b) => b.id === selBranch)?.name || "Şube"} year={year} month={month} legalType={legalType} sector={sector} onBack={() => setSelBranch(null)} />
        ) : (
          <div>
            <div style={{ background: "#0A0A0A", borderRadius: 16, padding: "18px 20px", boxShadow: "0 4px 18px rgba(0,0,0,0.13)", marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{ fontSize: 16, fontWeight: 700, color: "#fff" }}>Şubeler</span>
                <span style={{ fontSize: 11.5, color: "#2EE06A" }}>{AYLAR_F[month - 1]} {year} · salt-okunur</span>
              </div>
              <div style={{ fontSize: 12.5, color: "#2EE06A", marginTop: 5, opacity: 0.9 }}>Bir şube seç — o şubenin kendi gelir/gideri açılır.</div>
            </div>
            {branches.length === 0 ? (
              <div style={{ ...card, fontSize: 12.5, color: C.textFaint, padding: "28px 0", textAlign: "center" }}>Şube bulunamadı.</div>
            ) : (
              <div style={{ ...card, padding: "6px 8px" }}>
                {branches.map((b, idx) => (
                  <div key={b.id} onClick={() => setSelBranch(b.id)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 12px", borderTop: idx === 0 ? "none" : `1px solid ${C.borderSoft}`, cursor: "pointer", borderRadius: 10 }} onMouseEnter={(e) => (e.currentTarget.style.background = C.surface)} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ width: 38, height: 38, borderRadius: 10, background: C.greenSoft, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><i className="ti ti-building-store" style={{ fontSize: 18, color: C.greenDark }} aria-hidden="true" /></div>
                      <div style={{ minWidth: 0 }}><div style={{ fontSize: 14, fontWeight: 600, color: C.ink }}>{b.name}</div>{b.address ? <div style={{ fontSize: 12, color: C.textHint, marginTop: 1 }}>{b.address}</div> : null}</div>
                    </div>
                    <span style={{ fontSize: 12.5, color: C.greenDark, fontWeight: 500, display: "flex", alignItems: "center", gap: 4, whiteSpace: "nowrap" }}>Finansı gör <i className="ti ti-chevron-right" style={{ fontSize: 15 }} aria-hidden="true" /></span>
                  </div>
                ))}
              </div>
            )}
        ...
        <>
          <div style={{ background: "#0A0A0A", borderRadius: 18, padding: "20px 22px", boxShadow: "0 4px 18px rgba(0,0,0,0.13)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
                <div style={{ width: 42, height: 42, borderRadius: 11, background: "rgba(34,197,94,0.13)", display: "flex", alignItems: "center", justifyContent: "center" }}><i className="ti ti-building-bank" style={{ fontSize: 22, color: "#2EE06A" }} aria-hidden="true" /></div>
                <div><div style={{ fontSize: 18, fontWeight: 700, color: "#fff", lineHeight: 1.1 }}>Marka Finansı</div><div style={{ fontSize: 12, color: "#2EE06A", marginTop: 2 }}>{AYLAR_F[month - 1]} {year} · {LEGAL_LABELS[legalType] || "Şirket"} · {SECTOR_LABELS[sector] || "Diğer"}</div></div>
              </div>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "rgba(255,255,255,0.06)", borderRadius: 10, padding: "5px 6px" }}>
                <i onClick={prevMonth} className="ti ti-chevron-left" style={{ fontSize: 16, color: "rgba(255,255,255,0.7)", cursor: "pointer" }} aria-hidden="true" />
                <span style={{ fontSize: 12.5, color: "#fff", fontWeight: 500, minWidth: 100, textAlign: "center" }}>{AYLAR_F[month - 1]} {year}</span>
                <i onClick={nextMonth} className="ti ti-chevron-right" style={{ fontSize: 16, color: atCurrent ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.7)", cursor: atCurrent ? "default" : "pointer" }} aria-hidden="true" />
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1.05fr 1fr 1.05fr 1fr 1.25fr", gap: 9 }}>
              <KpiCell icon="ti-arrow-down-left" iconColor="#2EE06A" label="Marka geliri" value={fmtTLf(income)} sub={`${incomeCats.length} gelir kalemi`} />
              <KpiCell icon="ti-arrow-up-right" iconColor="#E0816F" label="Giderler" value={fmtTLf(expense)} sub="merkez" />
              <KpiCell icon="ti-building-bank" iconColor="#B6A6DC" label="Devlet ve vergi" value={fmtTLf(tax)} sub="tahmini" />
              <KpiCell icon="ti-sum" iconColor="#E0B95C" label="Toplam gider" value={fmtTLf(totalExpense)} sub="" />
              <div style={{ background: "rgba(34,197,94,0.10)", border: "1px solid rgba(34,197,94,0.25)", borderRadius: 12, padding: "13px 15px" }}>
                <div style={{ fontSize: 11, color: "#2EE06A", display: "flex", alignItems: "center", gap: 5, marginBottom: 7 }}><i className="ti ti-trending-up" style={{ fontSize: 14 }} aria-hidden="true" />Net kâr</div>
                <div style={{ fontSize: 23, fontWeight: 700, color: net >= 0 ? "#2EE06A" : "#E0816F" }}>{fmtTLf(net)}</div>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", marginTop: 3 }}>vergi sonrası{income > 0 ? ` · marj %${margin.toFixed(0)}` : ""}</div>
              </div>
            </div>
          </div>

          {loading ? (
            <div style={{ ...card, marginTop: 14, textAlign: "center", color: C.textFaint, fontSize: 13 }}>Yükleniyor…</div>
          ) : isEmpty ? (
            <div style={{ ...card, marginTop: 14, textAlign: "center", padding: "44px 20px" }}>
              <i className="ti ti-coin" style={{ fontSize: 30, color: C.textHint }} aria-hidden="true" />
              <div style={{ fontSize: 14, fontWeight: 600, color: C.ink, marginTop: 12 }}>Henüz marka geliri veya gideri girilmedi</div>
              <div style={{ fontSize: 12.5, color: C.textFaint, marginTop: 4, maxWidth: 380, marginLeft: "auto", marginRight: "auto" }}>Gelir / Gider menüsünden royalti, hammadde satışı, franchise bedeli gibi kalemler ekleyip kayıt girince burada görünecek.</div>
            </div>
          ) : (
            <>
              <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 14, marginTop: 14 }}>
                <div style={card}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                    <span style={{ fontSize: 13.5, fontWeight: 600, color: C.ink }}>Gelir kompozisyonu</span>
                    <span style={{ fontSize: 10.5, color: C.textHint, display: "flex", alignItems: "center", gap: 4 }}><i className="ti ti-arrows-exchange" style={{ fontSize: 13 }} aria-hidden="true" />Gelir / Gider'den tanımlanır</span>
                  </div>
                  {donut.length === 0 ? (<div style={{ fontSize: 12.5, color: C.textFaint, padding: "20px 0", textAlign: "center" }}>Gelir kalemi yok</div>) : (
                    <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                      <div style={{ position: "relative", width: 140, height: 140, flexShrink: 0 }}>
                        <ResponsiveContainer width={140} height={140}><PieChart><Pie data={donut} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={44} outerRadius={64} paddingAngle={donut.length > 1 ? 2 : 0} stroke="none">{donut.map((d, i) => <Cell key={i} fill={d.color} />)}</Pie></PieChart></ResponsiveContainer>
                        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}><div style={{ fontSize: 10, color: C.textFaint }}>Toplam</div><div style={{ fontSize: 15, fontWeight: 700, color: C.ink }}>{fmtCompact(income)}</div></div>
                      </div>
                      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 9, minWidth: 0 }}>
                        {donut.map((d, i) => (<div key={i} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12 }}><span style={{ width: 9, height: 9, borderRadius: 3, background: d.color, flexShrink: 0 }} /><span style={{ color: C.ink, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.name}</span><span style={{ color: C.textMuted, fontWeight: 500 }}>{fmtTLf(d.value)}</span></div>))}
                      </div>
                    </div>
                  )}
                </div>
                <div style={card}>
                  <div style={{ fontSize: 13.5, fontWeight: 600, color: C.ink, marginBottom: 14 }}>Aylık karşılaştırma</div>
                  <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-around", height: 132, borderBottom: `1px solid ${C.border}` }}>
                    {bars.map((b, i) => { const h = Math.max(4, Math.round((Math.abs(b.val) / barMax) * 120)); return (<div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, height: "100%", justifyContent: "flex-end" }}><span style={{ fontSize: 10, color: C.textMuted }}>{fmtCompact(b.val)}</span><div style={{ width: 42, height: h, background: b.color, borderRadius: "5px 5px 0 0" }} /></div>); })}
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-around", marginTop: 7 }}>{bars.map((b, i) => <span key={i} style={{ fontSize: 11, color: C.textMuted, width: 60, textAlign: "center" }}>{b.label}</span>)}</div>
                </div>
              </div>
              <div style={{ ...card, marginTop: 14 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  <span style={{ fontSize: 13.5, fontWeight: 600, color: C.ink }}>Merkez giderleri</span>
                  <span style={{ fontSize: 12, color: C.dangerInk, background: C.dangerBg, borderRadius: 7, padding: "3px 9px", fontWeight: 500 }}>{fmtTLf(expense)}</span>
                </div>
                {expenseCats.length === 0 ? (<div style={{ fontSize: 12.5, color: C.textFaint, padding: "14px 0" }}>Merkez gideri girilmemiş.</div>) : expenseCats.map((c, i) => (<div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 0", borderTop: `1px solid ${C.borderSoft}`, fontSize: 13 }}><span style={{ color: C.ink }}>{c.name}</span><span style={{ color: C.textMuted, fontWeight: 500 }}>{fmtTLf(c.total)}</span></div>))}
              </div>
            </>
          )}
          <div style={{ marginTop: 12, background: C.warnBg, border: `1px solid ${C.warnBorder}`, borderRadius: 12, padding: "11px 14px", fontSize: 11.5, color: C.warnInk, display: "flex", gap: 8 }}>
            <i className="ti ti-alert-triangle" style={{ fontSize: 15, marginTop: 1, flexShrink: 0 }} aria-hidden="true" />
            <span><b>Şube operasyonel gelir/gideri burada yok</b> — onlar Şubeler sekmesinde. Devlet ve vergi tahminidir ({LEGAL_LABELS[legalType] || "şirket"}, sektör {SECTOR_LABELS[sector] || "diğer"}). Net = Gelir − Giderler − Devlet/Vergi. Salt-okunur; kalemler Gelir / Gider'den tanımlanır.</span>
          </div>
        </>
      )}
    </div>
...
function KpiCell({ icon, iconColor, label, value, sub }: { icon: string; iconColor: string; label: string; value: string; sub: string }) {
  return (
    <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: "13px 14px" }}>
      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", display: "flex", alignItems: "center", gap: 5, marginBottom: 7 }}><i className={`ti ${icon}`} style={{ fontSize: 14, color: iconColor }} aria-hidden="true" />{label}</div>
...
// ── Şube finans detayı — manager Finans görünümünün birebir owner karşılığı (salt-okunur) ──
function BranchFinanceDetail({ token, branchId, branchName, year, month, legalType, sector, onBack }: { token: string; branchId: string; branchName: string; year: number; month: number; legalType: string; sector: string; onBack: () => void; }) {
  const headers = { Authorization: `Bearer ${token}` };
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get(`${API_URL}/finance/entries?year=${year}&month=${month}&branch_id=${branchId}&t=${Date.now()}`, { headers }),
      axios.get(`${API_URL}/payroll/branch/${branchId}/${year}/${month}?t=${Date.now()}`, { headers }).catch(() => ({ data: [] })),
    ]).then(([e, p]: any[]) => {
      if (!ok) return;
      setEntries(e.data || []);
      const rows = Array.isArray(p.data) ? p.data : (p.data?.rows || []);
      setPayroll(rows.reduce((s: number, r: any) => s + (r.employer_cost || 0), 0));
      setLoading(false);
  ...
  const gelir = incomeEntries.reduce((s, e) => s + (e.amount || 0), 0);
  const expenseOps = expenseEntries.reduce((s, e) => s + (e.amount || 0), 0);
  const giderler = expenseOps + payroll;
  const preTax = gelir - giderler;
  const tax = computeFinTax(legalType, sector, preTax);
  const toplamGider = giderler + tax;
  const net = gelir - toplamGider;
  const margin = gelir > 0 ? (net / gelir) * 100 : 0;

  const incMap: Record<string, number> = {};
  incomeEntries.forEach((e) => { const k = e.category_name || "—"; incMap[k] = (incMap[k] || 0) + (e.amount || 0); });
  const incomeRows = Object.entries(incMap).map(([name, total]) => ({ name, total }));
  const expMap: Record<string, number> = {};
  expenseEntries.forEach((e) => { const k = e.category_name || "—"; expMap[k] = (expMap[k] || 0) + (e.amount || 0); });
  const expenseRows = Object.entries(expMap).map(([name, total]) => ({ name, total }));

  // "Para nereye gidiyor" — cikis kompozisyonu
  const OUT_RENK = ["#C0564B", "#E0913C", "#E8B84B", "#D98C5F", "#C98A3A"];
  const outflow: { name: string; value: number; color: string }[] = [];
  expenseRows.forEach((c, i) => outflow.push({ name: c.name, value: c.total, color: OUT_RENK[i % OUT_RENK.length] }));
  if (payroll > 0) outflow.push({ name: "Personel maliyeti", value: payroll, color: "#5B8A72" });
  if (tax > 0) outflow.push({ name: "Devlet ve vergi (tahmini)", value: tax, color: "#7C6B9A" });
  const CIRC = 2 * Math.PI * 42.5;

  const barMax = Math.max(gelir, giderler, tax, Math.abs(net), 1);
  const bars = [{ label: "Gelir", val: gelir, color: "#1FA85A" }, { label: "Giderler", val: giderler, color: "#C0564B" }, { label: "Devlet/Vergi", val: tax, color: "#7C6B9A" }, { label: "Net", val: net, color: net >= 0 ? "#1FA85A" : "#C0564B" }];

  const card: React.CSSProperties = { background: C.bg, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, boxShadow: "0 1px 3px rgba(0,0,0,0.03)" };

  return (
    <div>
      <div onClick={onBack} style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12.5, color: C.greenDark, fontWeight: 500, cursor: "pointer", marginBottom: 12 }}><i className="ti ti-chevron-left" style={{ fontSize: 16 }} aria-hidden="true" /> Şubeler</div>

      {/* Koyu band — 5 KPI */}
      <div style={{ background: "#0A0A0A", borderRadius: 18, padding: "20px 22px", boxShadow: "0 4px 18px rgba(0,0,0,0.13)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 13, marginBottom: 18 }}>
          <div style={{ width: 42, height: 42, borderRadius: 11, background: "rgba(34,197,94,0.13)", display: "flex", alignItems: "center", justifyContent: "center" }}><i className="ti ti-building-store" style={{ fontSize: 22, color: "#2EE06A" }} aria-hidden="true" /></div>
          <div><div style={{ fontSize: 18, fontWeight: 700, color: "#fff", lineHeight: 1.1 }}>{branchName}</div><div style={{ fontSize: 12, color: "#2EE06A", marginTop: 2 }}>{AYLAR_F[month - 1]} {year} · {LEGAL_LABELS[legalType] || "Şirket"} · {SECTOR_LABELS[sector] || "Diğer"}</div></div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1.05fr 1.15fr 1.05fr 1fr 1.25fr", gap: 9 }}>
          <KpiCell icon="ti-arrow-down-left" iconColor="#2EE06A" label="Toplam gelir" value={fmtTLf(gelir)} sub="" />
          <KpiCell icon="ti-arrow-up-right" iconColor="#E0816F" label="Giderler" value={fmtTLf(giderler)} sub="operasyonel + personel" />
          <KpiCell icon="ti-building-bank" iconColor="#B6A6DC" label="Devlet ve vergi" value={fmtTLf(tax)} sub="tahmini" />
          <KpiCell icon="ti-sum" iconColor="#E0B95C" label="Toplam gider" value={fmtTLf(toplamGider)} sub="" />
          <div style={{ background: "rgba(34,197,94,0.10)", border: "1px solid rgba(34,197,94,0.25)", borderRadius: 12, padding: "13px 15px" }}>
            <div style={{ fontSize: 11, color: "#2EE06A", display: "flex", alignItems: "center", gap: 5, marginBottom: 7 }}><i className="ti ti-trending-up" style={{ fontSize: 14 }} aria-hidden="true" />Net kâr</div>
            <div style={{ fontSize: 23, fontWeight: 700, color: net >= 0 ? "#2EE06A" : "#E0816F" }}>{fmtTLf(net)}</div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", marginTop: 3 }}>vergi sonrası{gelir > 0 ? ` · marj %${margin.toFixed(0)}` : ""}</div>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          {/* Para nereye gidiyor + Aylik karsilastirma */}
          <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 14, marginTop: 14 }}>
            <div style={card}>
              <div style={{ fontSize: 13.5, fontWeight: 600, color: C.ink, marginBottom: 14 }}>Para nereye gidiyor</div>
              {outflow.length === 0 ? (<div style={{ fontSize: 12.5, color: C.textFaint, padding: "20px 0", textAlign: "center" }}>Gider yok</div>) : (
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ position: "relative", width: 140, height: 140, flexShrink: 0 }}>
                    <svg viewBox="0 0 120 120" width={140} height={140}>
                      <g transform="rotate(-90 60 60)" fill="none" strokeWidth={15}>
                        {(() => { let acc = 0; return outflow.map((s, i) => { const len = toplamGider > 0 ? (s.value / toplamGider) * CIRC : 0; const el = <circle key={i} cx={60} cy={60} r={42.5} stroke={s.color} strokeDasharray={`${len} ${CIRC - len}`} strokeDashoffset={-acc} />; acc += len; return el; }); })()}
                      </g>
                    </svg>
                    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}><div style={{ fontSize: 10, color: C.textFaint }}>Toplam çıkış</div><div style={{ fontSize: 14, fontWeight: 700, color: C.ink }}>{fmtCompact(toplamGider)}</div></div>
                  </div>
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10, minWidth: 0 }}>
                    {outflow.map((s, i) => (<div key={i} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12 }}><span style={{ width: 9, height: 9, borderRadius: 3, background: s.color, flexShrink: 0 }} /><span style={{ color: C.ink, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.name}</span><span style={{ color: C.textMuted, fontWeight: 500 }}>{fmtTLf(s.value)}</span></div>))}
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Gelir kategorileri + Gider kategorileri */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 14 }}>
            <div style={card}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 13.5, fontWeight: 600, color: C.ink }}>Gelir kategorileri</span>
                <span style={{ fontSize: 12, color: C.greenDark, background: C.greenSoft, borderRadius: 7, padding: "3px 9px", fontWeight: 500 }}>{fmtTLf(gelir)}</span>
              </div>
              {incomeRows.length === 0 ? (<div style={{ fontSize: 12.5, color: C.textFaint, padding: "14px 0" }}>Gelir kaydı yok.</div>) : incomeRows.map((c, i) => (<div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 0", borderTop: `1px solid ${C.borderSoft}`, fontSize: 13 }}><span style={{ color: C.ink }}>{c.name}</span><span style={{ color: C.textMuted, fontWeight: 500 }}>{fmtTLf(c.total)}</span></div>))}
            </div>
            <div style={card}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 13.5, fontWeight: 600, color: C.ink }}>Gider kategorileri</span>
                <span style={{ fontSize: 12, color: C.dangerInk, background: C.dangerBg, borderRadius: 7, padding: "3px 9px", fontWeight: 500 }}>{fmtTLf(giderler)}</span>
              </div>
              {expenseRows.map((c, i) => (<div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 0", borderTop: `1px solid ${C.borderSoft}`, fontSize: 13 }}><span style={{ color: C.ink }}>{c.name}</span><span style={{ color: C.textMuted, fontWeight: 500 }}>{fmtTLf(c.total)}</span></div>))}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 0", borderTop: `1px solid ${C.borderSoft}`, fontSize: 13 }}><span style={{ color: C.textFaint }}>Personel maliyeti (bordro)</span><span style={{ color: C.textFaint, fontWeight: 500 }}>{fmtTLf(payroll)}</span></div>
            </div>
          </div>

          <div style={{ marginTop: 12, background: C.warnBg, border: `1px solid ${C.warnBorder}`, borderRadius: 12, padding: "11px 14px", fontSize: 11.5, color: C.warnInk, display: "flex", gap: 8 }}>
            <i className="ti ti-alert-triangle" style={{ fontSize: 15, marginTop: 1, flexShrink: 0 }} aria-hidden="true" />
            <span><b>Devlet ve vergi tahminidir.</b> {LEGAL_LABELS[legalType] || "Şirket"} · {SECTOR_LABELS[sector] || "Diğer"}. Kurumlar/gelir vergisi gerçekte yıllık ve şirket bazında tahakkuk eder; buradaki rakam bu şubenin bu ayki kârına göre projeksiyondur. Net = Gelir − Giderler − Devlet/Vergi. Salt-okunur; kayıtlar şubenin Gelir / Gider'inden gelir.</span>
          </div>
        </>
      )}
    </div>
...
const fmtTLL = (n: number) => "₺" + (n ?? 0).toLocaleString("tr-TR");
...
function LedgerPage({ token, companyId }: { token: string; companyId: string }) {
  ...
  const loadBranches = async () => {
    try {
      ...
      setBranches(r.data.map((b: any) => ({ id: b.id, name: b.name })));
    } catch { setBranches([]); }
  };
  const loadCats = async () => {
    try { const r = await axios.get(`${API_URL}/finance/categories?kind=${kind}&scope=brand&t=${Date.now()}`, { headers }); setCats(r.data); }
    catch { setCats([]); }
  };
  const loadEntries = async () => {
    setLoading(true);
    try {
      let url = `${API_URL}/finance/entries?year=${year}&month=${month}&kind=${kind}&include_deleted=${showDeleted}&scope=brand&t=${Date.now()}`;
  ...
  const addCat = async () => {
    ...
    if (!name) return;
    try { await axios.post(`${API_URL}/finance/categories`, { name, kind, scope: "brand" }, { headers }); setNewCatName(""); await loadCats(); } catch { /* */ }
  };
  ...
  const accent = kind === "expense" ? C.warnInk : C.greenDark;
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          <div style={{ display: "flex", alignItems: "center", background: C.surface, border: "0.5px solid #E6E4DD", borderRadius: 9, overflow: "hidden" }}>
            <button onClick={prevMonth} aria-label="Önceki ay" style={{ border: "none", background: "transparent", padding: "7px 10px", cursor: "pointer", color: C.textFaint, display: "flex" }}><i className="ti ti-chevron-left" style={{ fontSize: 14 }} aria-hidden="true" /></button>
            <span style={{ fontSize: 12.5, fontWeight: 600, padding: "0 6px", minWidth: 100, textAlign: "center" }}>{AYLAR_L[month - 1]} {year}</span>
            <button onClick={nextMonth} aria-label="Sonraki ay" style={{ border: "none", background: "transparent", padding: "7px 10px", cursor: atCurrent ? "not-allowed" : "pointer", color: atCurrent ? "#E0DDD6" : C.textFaint, display: "flex" }}><i className="ti ti-chevron-right" style={{ fontSize: 14 }} aria-hidden="true" /></button>
          </div>
      ...
      {showCatMgr && (
        <Modal title={`${kind === "expense" ? "Gider" : "Gelir"} Kategorileri`} onClose={() => setShowCatMgr(false)}>
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            <input value={newCatName} onChange={(e) => setNewCatName(e.target.value)} placeholder={kind === "expense" ? "Kira, Reklam, Nakliye…" : "Royalty, Kira geliri…"} onKeyDown={(e) => { if (e.key === "Enter") addCat(); }} style={{ flex: 1, fontSize: 13, border: `0.5px solid ${C.border}`, borderRadius: 8, padding: "9px 12px" }} />
            <button onClick={addCat} style={{ fontSize: 12.5, fontWeight: 600, color: "#fff", background: C.green, border: "none", padding: "9px 16px", borderRadius: 8, cursor: "pointer" }}>Ekle</button>
          </div>
          {cats.length === 0 ? (
            <div style={{ fontSize: 12, color: C.textHint, padding: "10px 0", textAlign: "center" }}>Henüz kategori yok. Yukarıdan ekleyin.</div>
          ) : (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {cats.map((c) => (
                <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 7, background: C.surface, border: `0.5px solid ${C.border}`, borderRadius: 9, padding: "7px 11px" }}>
                  <span style={{ fontSize: 12.5 }}>{c.name}</span>
                  <i className="ti ti-x" onClick={() => delCat(c.id)} style={{ fontSize: 13, color: C.textHint, cursor: "pointer" }} aria-hidden="true" />
                </div>
              ))}
            </div>
          )}
          <div style={{ fontSize: 10.5, color: C.textHint, marginTop: 13, lineHeight: 1.5 }}>Kategori silinince kayıtları silinmez; kayıtlar kategori bağı korunarak kalır.</div>
        </Modal>
      )}
      ...
      {showEntry && (
        <Modal title={editId ? `${kind === "expense" ? "Gider" : "Gelir"} Kaydını Düzenle` : `Yeni ${kind === "expense" ? "Gider" : "Gelir"} Kaydı`} onClose={() => setShowEntry(false)}>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 11 }}>
            <div style={{ flex: 1 }}>
              <label style={{ fontSize: 11, color: C.textMuted, display: "block", marginBottom: 4 }}>Tutar (₺)</label>
              <input type="number" value={entryForm.amount} onChange={(e) => setEntryForm({ ...entryForm, amount: e.target.value })} placeholder="0,00" style={{ width: "100%", fontSize: 12.5, border: `0.5px solid ${C.border}`, borderRadius: 8, padding: "9px 11px", boxSizing: "border-box" }} />
            </div>
            <div style={{ flex: 1 }}>
              <label style={{ fontSize: 11, color: C.textMuted, display: "block", marginBottom: 4 }}>Tarih</label>
              <input type="date" value={entryForm.entry_date} onChange={(e) => setEntryForm({ ...entryForm, entry_date: e.target.value })} style={{ width: "100%", fontSize: 12.5, border: `0.5px solid ${C.border}`, borderRadius: 8, padding: "9px 11px", boxSizing: "border-box" }} />
            </div>
          </div>
          <div style={{ marginBottom: 11 }}>
            <label style={{ fontSize: 11, color: C.textMuted, display: "block", marginBottom: 4 }}>Şube <span style={{ color: C.textHint }}>(boş = marka geneli)</span></label>
            <select value={entryForm.branch_id} onChange={(e) => setEntryForm({ ...entryForm, branch_id: e.target.value })} style={{ width: "100%", fontSize: 12.5, border: `0.5px solid ${C.border}`, borderRadius: 8, padding: "9px 11px", background: "#fff" }}>
              <option value="">Marka geneli</option>
              {branches.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
            </select>
          </div>
...
function PayrollPage({ token, companyId, jumpBranch, clearJump, tab, setTab }: { token: string; companyId: string; jumpBranch?: string | null; clearJump?: () => void; tab: "merkez" | "branches" | "franchise"; setTab: (t: "merkez" | "branches" | "franchise") => void }) {
  const headers = { Authorization: `Bearer ${token}` };
  const today = new Date();
  const [year, setYear] = useState(today.getFullYear());
  const [month, setMonth] = useState(today.getMonth() + 1);

  // Merkez (sube-siz personel)
  const [merkezRows, setMerkezRows] = useState<PayRow[]>([]);
  const [merkezLoading, setMerkezLoading] = useState(false);
  ...
  // Franchise (sadece toplam)
  const [franTotals, setFranTotals] = useState<any[]>([]);
  const [franLoading, setFranLoading] = useState(false);

  const [slip, setSlip] = useState<{ row: PayRow; p: Payslip } | null>(null);

  // ── Merkez ──
  const loadMerkez = async (y: number, m: number) => {
    setMerkezLoading(true); setRunErr("");
    try { const r = await axios.get(`${API_URL}/payroll/merkez/${y}/${m}?t=${Date.now()}`, { headers }); setMerkezRows(r.data); }
    catch { setMerkezRows([]); }
    setMerkezLoading(false);
  };
  useEffect(() => { if (tab === "merkez") loadMerkez(year, month); /* eslint-disable-next-line */ }, [tab, year, month]);

  const runMerkez = async () => {
    if (running) return;
    setRunning(true); setRunErr("");
    try {
      const r = await axios.post(`${API_URL}/payroll/run-merkez`, { year, month }, { headers });
      const fails = r.data.filter((x: any) => !x.ok);
      if (fails.length) setRunErr(`${fails.length} çalışan hesaplanamadı (maaş tanımsız olabilir).`);
      await loadMerkez(year, month);
    } catch (e: any) { const d = e.response?.data?.detail; setRunErr(typeof d === "string" ? d : "Dönem çalıştırılamadı."); }
    setRunning(false);
  };
  const runOneMerkez = async (employee_id: string) => {
    setRunErr("");
    try { await axios.post(`${API_URL}/payroll/run-through`, { employee_id, year, month }, { headers }); await loadMerkez(year, month); }
    catch (e: any) { const d = e.response?.data?.detail; setRunErr(typeof d === "string" ? d : "Hesaplanamadı."); }
  };

  // ── Subeler ──
  const loadBranches = async () => {
    try { const r = await axios.get(`${API_URL}/companies/${companyId}/branches?t=${Date.now()}`, { headers }); setBranches(r.data.map((b: any) => ({ id: b.id, name: b.name, address: b.address }))); }
    catch { setBranches([]); }
  ...
  const loadCounts = async (y: number, m: number, list: PayBranch[]) => {
    ...
    await Promise.all(list.map(async (b) => {
      try { const r = await axios.get(`${API_URL}/payroll/branch/${b.id}/${y}/${m}?t=${Date.now()}`, { headers }); c[b.id] = { total: r.data.length, done: r.data.filter((x: PayRow) => x.payslip).length }; }
      catch { c[b.id] = { total: 0, done: 0 }; }
    }));
    setCounts(c);
  };
  useEffect(() => { if (tab === "branches" && branchesLoaded && branches.length) loadCounts(year, month, branches); /* eslint-disable-next-line */ }, [tab, branchesLoaded, branches.length, year, month]);
  ...
  useEffect(() => { if (selected && tab === "branches") loadRows(selected, year, month); /* eslint-disable-next-line */ }, [year, month]);

  // jumpBranch -> Subeler sekmesine git, subeyi sec
  useEffect(() => {
  ...
  // ── Franchise ──
  const loadFran = async (y: number, m: number) => {
    setFranLoading(true);
    try { const r = await axios.get(`${API_URL}/payroll/franchise-totals/${y}/${m}?t=${Date.now()}`, { headers }); setFranTotals(r.data); }
    catch { setFranTotals([]); }
    setFranLoading(false);
  };
  useEffect(() => { if (tab === "franchise") loadFran(year, month); /* eslint-disable-next-line */ }, [tab, year, month]);

  // ── shared month nav ──
  const prevMonth = () => { if (month === 1) { setMonth(12); setYear(year - 1); } else setMonth(month - 1); };
  const nextMonth = () => { const isCur = year === today.getFullYear() && month === today.getMonth() + 1; if (isCur) return; if (month === 12) { setMonth(1); setYear(year + 1); } else setMonth(month + 1); };
  const atCurrent = year === today.getFullYear() && month === today.getMonth() + 1;

  const activeRows: PayRow[] = tab === "merkez" ? merkezRows : rows;
  const activeLoading = tab === "merkez" ? merkezLoading : rowsLoading;
  const readOnly = tab === "branches";
  const done = activeRows.filter((r) => r.payslip);
  ...
  const subtitle = tab === "merkez" ? "Markanın kendi (şubesiz) personeli. Dönemi hesapla, fişleri incele."
    : tab === "branches" ? "Şube bordrosunu şube yöneticisi çalıştırır. Marka panelinde salt-okunur."
    : "Franchise bordrosu işletmeye özeldir — yalnızca toplam işçilik maliyeti görünür.";
  const bandTitle = tab === "merkez" ? "Merkez (şubesiz personel)" : selected?.name || "";
  const slipContext = tab === "merkez" ? "Merkez" : (selected?.name || "");
  ...
  const renderBandAndTable = () => (
    <>
      <div style={{ background: C.ink, borderRadius: 14, padding: "16px 19px", marginBottom: 13, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <div style={{ fontSize: 10.5, color: "#7C7A75", marginBottom: 4 }}>{AYLAR[month - 1]} {year} · {bandTitle} · İşveren Toplam Maliyeti</div>
          <div style={{ fontSize: 25, fontWeight: 600, color: "#fff", letterSpacing: "-0.02em" }}>{activeLoading ? "—" : fmtTL2(totCost)}</div>
        </div>
        <div style={{ display: "flex", gap: 22, textAlign: "right" }}>
          <div><div style={{ fontSize: 14, fontWeight: 600, color: "#fff" }}>{activeLoading ? "—" : fmtTL2(totGross)}</div><div style={{ fontSize: 9.5, color: "#7C7A75", marginTop: 2 }}>toplam brüt</div></div>
          <div><div style={{ fontSize: 14, fontWeight: 600, color: "#2EE06A" }}>{activeLoading ? "—" : fmtTL2(totNet)}</div><div style={{ fontSize: 9.5, color: "#7C7A75", marginTop: 2 }}>çalışana net</div></div>
          <div><div style={{ fontSize: 14, fontWeight: 600, color: "#E0A82E" }}>{activeLoading ? "—" : fmtTL2(totState)}</div><div style={{ fontSize: 9.5, color: "#7C7A75", marginTop: 2 }}>devlete giden</div></div>
        </div>
      </div>

      {runErr && !readOnly && <div style={{ fontSize: 12, color: C.warnInk, background: C.warnBg, border: `0.5px solid ${C.warnBorder}`, borderRadius: 9, padding: "8px 12px", marginBottom: 11 }}>⚠ {runErr}</div>}

      {activeLoading ? (
        <div style={{ fontSize: 12.5, color: C.textHint, padding: "20px 0", textAlign: "center" }}>Yükleniyor…</div>
      ) : activeRows.length === 0 ? (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "30px 0", color: C.textHint }}>
          <i className="ti ti-users" style={{ fontSize: 22 }} aria-hidden="true" /><span style={{ fontSize: 12.5, textAlign: "center", maxWidth: 320 }}>{tab === "merkez" ? "Şubesiz personel yok. Personel'den şubesiz çalışan ekleyince burada görünür." : "Bu şubede aktif personel yok."}</span>
        </div>
      ) : (
        <>
            ...
            {activeRows.map((r) => {
              ...
              return (
                <div key={r.employee_id} onClick={() => { if (p) setSlip({ row: r, p }); }} style={{ background: C.bg, border: p ? `0.5px solid ${C.border}` : `0.5px dashed #E0DACE`, borderRadius: 11, padding: "11px 15px", cursor: p ? "pointer" : "default", display: "grid", gridTemplateColumns: "1.9fr 1fr 1fr 0.95fr 36px", gap: 10, alignItems: "center" }}>
                  ...
                  {readOnly ? (
                    p ? <i className="ti ti-chevron-right" style={{ fontSize: 15, color: "#D4D0C8", margin: "0 auto" }} aria-hidden="true" /> : <i className="ti ti-lock" style={{ fontSize: 14, color: C.textHint, margin: "0 auto" }} aria-hidden="true" />
                  ) : p ? (
                    ...
                    <button onClick={(e) => { e.stopPropagation(); runOneMerkez(r.employee_id); }} aria-label="Bordroyu çalıştır" style={{ width: 30, height: 30, borderRadius: 8, border: "none", background: C.green, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto" }}><i className="ti ti-player-play" style={{ fontSize: 13, color: "#fff" }} aria-hidden="true" /></button>
                  )}
                </div>
              );
            })}
    ...
    <>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
        <div>
          ...
          <div style={{ fontSize: 12.5, color: C.textMuted, marginTop: 2 }}>{subtitle}</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {monthNav}
          {tab === "merkez" && (
            <button onClick={runMerkez} disabled={running || merkezRows.length === 0} style={{ fontSize: 12.5, fontWeight: 600, color: "#fff", background: running || merkezRows.length === 0 ? "#8A867F" : C.ink, border: "none", padding: "9px 16px", borderRadius: 10, cursor: running || merkezRows.length === 0 ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 7 }}>
          ...
          {tab === "branches" && (
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: C.neutralBg, border: `0.5px solid ${C.border}`, color: C.textFaint, borderRadius: 10, padding: "9px 14px", fontSize: 12.5, fontWeight: 500 }}><i className="ti ti-lock" style={{ fontSize: 15 }} aria-hidden="true" />Salt-okunur</div>
          )}
        </div>
      </div>
      <div style={{ height: 14 }} />
      ...
      {tab === "merkez" ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "0.8fr 1.7fr", gap: 18 }}>
          ...
          <div>
            {!selected ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, padding: "60px 0", color: C.textHint, border: `1px dashed ${C.border}`, borderRadius: 14 }}>
                <i className="ti ti-arrow-left" style={{ fontSize: 22 }} aria-hidden="true" /><span style={{ fontSize: 12.5 }}>Bordroyu görmek için bir şube seçin.</span>
              </div>
            ) : renderBandAndTable()}
          </div>
        </div>
        ...
        <div>
          {franLoading ? (
            <div style={{ fontSize: 12.5, color: C.textHint, padding: "40px 0", textAlign: "center" }}>Yükleniyor…</div>
          ) : franTotals.length === 0 ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "50px 0", color: C.textHint, border: `1px dashed ${C.border}`, borderRadius: 14 }}>
              <i className="ti ti-building-store" style={{ fontSize: 24 }} aria-hidden="true" /><span style={{ fontSize: 12.5 }}>Bağlı franchise yok.</span>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {franTotals.map((f) => (
                <div key={f.company_id} style={{ background: C.bg, border: `0.5px solid ${C.border}`, borderRadius: 14, padding: "16px 18px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
                    <div style={{ width: 42, height: 42, borderRadius: 11, background: C.franchiseSoft, display: "flex", alignItems: "center", justifyContent: "center" }}><i className="ti ti-building-store" style={{ fontSize: 20, color: C.franchise }} aria-hidden="true" /></div>
                    <div><div style={{ fontSize: 14.5, fontWeight: 600, color: C.ink }}>{f.name}</div><div style={{ fontSize: 12, color: C.textHint, marginTop: 1 }}>{f.employee_count} personel · {f.calculated_count}/{f.employee_count} hesaplandı</div></div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 11, color: C.textHint, display: "flex", alignItems: "center", gap: 5, justifyContent: "flex-end" }}><i className="ti ti-lock" style={{ fontSize: 13 }} aria-hidden="true" />Toplam işçilik maliyeti</div>
                    <div style={{ fontSize: 20, fontWeight: 700, color: C.ink, marginTop: 3 }}>{fmtTL2(f.total_employer_cost)}</div>
                  </div>
                </div>
              ))}
            </div>
          )}
          <div style={{ marginTop: 14, background: C.warnBg, border: `1px solid ${C.warnBorder}`, borderRadius: 12, padding: "11px 14px", fontSize: 11.5, color: C.warnInk, display: "flex", gap: 8 }}>
            <i className="ti ti-shield-lock" style={{ fontSize: 15, marginTop: 1, flexShrink: 0 }} aria-hidden="true" />
            <span><b>KVKK koruması.</b> Franchise ayrı işverendir. Marka yalnızca toplam işçilik maliyetini görür; çalışan adı, maaşı ve fişi paylaşılmaz.</span>
          </div>
        </div>
      )}
      ...
      {slip && (
        <Modal title={`${slip.row.first_name} ${slip.row.last_name} — ${AYLAR[slip.p.month - 1]} ${slip.p.year}`} onClose={() => setSlip(null)}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            <span style={{ fontSize: 11.5, color: C.textFaint }}>{slip.row.position || "—"} · {slipContext}</span>
          ...
          <div style={{ marginTop: 10, display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 10, color: C.textHint }}>
            ...
            {!readOnly && <span onClick={() => { runOneMerkez(slip.row.employee_id); setSlip(null); }} style={{ display: "flex", alignItems: "center", gap: 4, cursor: "pointer", color: C.greenDark }}><i className="ti ti-rotate" style={{ fontSize: 11 }} aria-hidden="true" />Yeniden çalıştır</span>}
          </div>
        </Modal>
      )}
    </>
...
function StaffPage({ token, companyId, jumpBranch, clearJump, tab, setTab }: { token: string; companyId: string; jumpBranch?: string | null; clearJump?: () => void; tab: "merkez" | "branches" | "franchise"; setTab: (t: "merkez" | "branches" | "franchise") => void }) {
  const headers = { Authorization: `Bearer ${token}` };

  // Merkez
  const [merkezEmps, setMerkezEmps] = useState<Employee[]>([]);
  const [merkezLoading, setMerkezLoading] = useState(false);
  ...
  const [branchesLoaded, setBranchesLoaded] = useState(false);
  const [selBranch, setSelBranch] = useState<StaffBranch | null>(null);
  const [branchEmps, setBranchEmps] = useState<Employee[]>([]);
  const [branchEmpLoading, setBranchEmpLoading] = useState(false);

  // Franchise
  const [franchises, setFranchises] = useState<{ id: string; name: string }[]>([]);
  const [franCounts, setFranCounts] = useState<Record<string, number>>({});
  const [selFran, setSelFran] = useState<{ id: string; name: string } | null>(null);
  const [franRoster, setFranRoster] = useState<any[]>([]);
  const [franRosterLoading, setFranRosterLoading] = useState(false);

  // Modallar (yalnizca Marka)
  ...
  const canManage = tab === "merkez";

  // ── Merkez ──
  const loadMerkez = async () => {
    setMerkezLoading(true);
    try { const r = await axios.get(`${API_URL}/employees/merkez?t=${Date.now()}`, { headers }); setMerkezEmps(r.data); }
    catch { setMerkezEmps([]); }
    setMerkezLoading(false);
  };
  useEffect(() => { if (tab === "merkez") loadMerkez(); /* eslint-disable-next-line */ }, [tab]);

  // ── Subeler ──
  const loadBranches = async () => {
    const list: StaffBranch[] = [];
    try { const r = await axios.get(`${API_URL}/companies/${companyId}/branches?t=${Date.now()}`, { headers }); r.data.forEach((b: any) => list.push({ id: b.id, name: b.name, address: b.address, kind: "brand" })); }
    catch {}
    setBranches(list);
    const c: Record<string, number> = {};
    await Promise.all(list.map(async (b) => { try { const r = await axios.get(`${API_URL}/employees/branch/${b.id}?t=${Date.now()}`, { headers }); c[b.id] = r.data.length; } catch { c[b.id] = 0; } }));
    setBranchCounts(c);
    setBranchesLoaded(true);
  };
  useEffect(() => { loadBranches(); /* eslint-disable-next-line */ }, [companyId]);

  const loadBranchEmps = async (branchId: string) => {
    setBranchEmpLoading(true);
    try { const r = await axios.get(`${API_URL}/employees/branch/${branchId}?t=${Date.now()}`, { headers }); setBranchEmps(r.data); }
    catch { setBranchEmps([]); }
    setBranchEmpLoading(false);
  };
  const selectBranch = (b: StaffBranch) => { setSelBranch(b); loadBranchEmps(b.id); };
  ...
  const loadFranchises = async () => {
    ...
    try {
      const r = await axios.get(`${API_URL}/oversight/subs?t=${Date.now()}`, { headers });
      r.data.filter((s: any) => s.link_type === "franchise").forEach((f: any) => list.push({ id: f.id, name: f.name }));
    } catch {}
    setFranchises(list);
    const c: Record<string, number> = {};
    await Promise.all(list.map(async (f) => { try { const r = await axios.get(`${API_URL}/employees/franchise-roster/${f.id}?t=${Date.now()}`, { headers }); c[f.id] = r.data.length; } catch { c[f.id] = 0; } }));
    setFranCounts(c);
  };
  useEffect(() => { if (tab === "franchise") loadFranchises(); /* eslint-disable-next-line */ }, [tab]);

  const loadFranRoster = async (subCompanyId: string) => {
    setFranRosterLoading(true);
    try { const r = await axios.get(`${API_URL}/employees/franchise-roster/${subCompanyId}?t=${Date.now()}`, { headers }); setFranRoster(r.data); }
    catch { setFranRoster([]); }
    setFranRosterLoading(false);
  };
  const selectFran = (f: { id: string; name: string }) => { setSelFran(f); loadFranRoster(f.id); };
  ...
  const addEmployee = async () => {
    ...
    try {
      const payload: any = { first_name: form.first_name, last_name: form.last_name, contract_type: form.contract_type, company_id: companyId }; // branch_id YOK = merkez
  ...
  const openDetail = (em: Employee) => {
    setDetail(em);
    setEdit({ first_name: em.first_name || "", last_name: em.last_name || "", phone: em.phone || "", position: em.position || "", department: em.department || "", base_salary: em.base_salary != null ? String(em.base_salary) : "", bank_iban: em.bank_iban || "" });
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/employees/${detail!.id}`, epayload, { headers }); setEditing(false); loadMerkez(); setDetail({ ...detail!, ...epayload });
    } catch (e: any) { setMsg(e.response?.data?.detail || "Güncellenemedi."); }
  };
  const terminate = async () => {
    ...
    try {
      ...
      setDetail(null); setTermPw(""); loadMerkez();
    } catch (e: any) {
      const d = e.response?.data?.detail;
  ...
  // ── employee satiri (Marka & Subeler ortak; clickable) ──
  const empRow = (em: Employee) => {
    ...
    return (
      <div key={em.id} onClick={() => openDetail(em)} style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 11, padding: "12px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 11 }}>
        ...
        {em.base_salary != null && <span style={{ fontSize: 11.5, color: C.textMuted, flexShrink: 0 }}>₺{Number(em.base_salary).toLocaleString("tr-TR")}</span>}
  ...
  return (
    <>
      ...
      {tab === "merkez" && (
        <>
          <div style={{ background: "#0A0A0A", borderRadius: 16, padding: "18px 20px", boxShadow: "0 4px 18px rgba(0,0,0,0.13)", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#fff" }}>Merkez personeli</div>
              <div style={{ fontSize: 12.5, color: "#2EE06A", marginTop: 5 }}>Genel merkez kadrosu — şubeye bağlı olmayan, doğrudan markaya bağlı çalışanlar.</div>
            </div>
            <button style={{ fontSize: 13, fontWeight: 600, color: "#fff", background: C.green, border: "none", padding: "9px 16px", borderRadius: 10, cursor: "pointer", display: "flex", alignItems: "center", gap: 7, whiteSpace: "nowrap" }} onClick={() => { setShowForm(true); setFormErr(""); }}><i className="ti ti-plus" style={{ fontSize: 16 }} aria-hidden="true" />Personel Ekle</button>
          </div>
          {merkezLoading ? (
            <div style={{ fontSize: 12.5, color: C.textHint, padding: "30px 0", textAlign: "center" }}>Yükleniyor…</div>
          ) : merkezEmps.length === 0 ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "50px 0", color: C.textHint, border: `1px dashed ${C.border}`, borderRadius: 14 }}>
              <i className="ti ti-user-plus" style={{ fontSize: 24 }} aria-hidden="true" /><span style={{ fontSize: 12.5, textAlign: "center", maxWidth: 340 }}>Henüz merkez personeli yok. "Personel Ekle" ile şubeye bağlı olmayan çalışan ekleyebilirsin.</span>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{merkezEmps.map((em) => empRow(em))}</div>
          )}
        </>
      )}
      ...
      {tab === "branches" && (
        <>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
            <div><div style={{ fontSize: 20, fontWeight: 700, color: C.ink }}>Şube personeli</div><div style={{ fontSize: 12.5, color: C.textMuted, marginTop: 3 }}>Şube personelini şube yöneticisi yönetir. Marka panelinde izlenir.</div></div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: C.neutralBg, border: `0.5px solid ${C.border}`, color: C.textFaint, borderRadius: 10, padding: "9px 14px", fontSize: 12.5, fontWeight: 500 }}><i className="ti ti-lock" style={{ fontSize: 15 }} aria-hidden="true" />Salt-okunur</div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "0.9fr 1.4fr", gap: 18 }}>
            <div>
              ...
              {!branchesLoaded ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "30px 0", color: C.textHint }}><i className="ti ti-building-off" style={{ fontSize: 22 }} aria-hidden="true" /><span style={{ fontSize: 12.5 }}>Henüz şube yok.</span></div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {branches.map((b) => {
                    const isSel = selBranch?.id === b.id;
                    ...
                    return (
                      <div key={b.id} onClick={() => selectBranch(b)} style={{ background: isSel ? C.bg : C.surface, border: isSel ? `1.5px solid ${C.green}` : `1px solid ${C.border}`, borderRadius: 11, padding: "12px 14px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                          ...
                          <span style={{ fontSize: 9, color: C.greenDark, background: C.greenSoft, padding: "2px 7px", borderRadius: 8, fontWeight: 600, whiteSpace: "nowrap", flexShrink: 0 }}>Markaya Ait</span>
                        </div>
            ...
            <div>
              {!selBranch ? (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, padding: "60px 0", color: C.textHint, border: `1px dashed ${C.border}`, borderRadius: 14 }}><i className="ti ti-arrow-left" style={{ fontSize: 22 }} aria-hidden="true" /><span style={{ fontSize: 12.5 }}>Personeli görmek için bir şube seçin.</span></div>
              ) : (
                <>
                  <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 12 }}>{selBranch.name} — Personel ({branchEmps.length})</div>
                  {branchEmpLoading ? (
                    <div style={{ fontSize: 12.5, color: C.textHint, padding: "20px 0", textAlign: "center" }}>Yükleniyor…</div>
                  ) : branchEmps.length === 0 ? (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "30px 0", color: C.textHint }}><i className="ti ti-user-off" style={{ fontSize: 22 }} aria-hidden="true" /><span style={{ fontSize: 12.5 }}>Bu şubede personel yok.</span></div>
                  ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{branchEmps.map((em) => empRow(em))}</div>
                  )}
                </>
              )}
            </div>
      ...
      {tab === "franchise" && (
        <>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
            <div><div style={{ fontSize: 20, fontWeight: 700, color: C.ink }}>Franchise personeli</div><div style={{ fontSize: 12.5, color: C.textMuted, marginTop: 3 }}>Ayrı işveren. Yalnızca isim ve pozisyon görünür.</div></div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: C.franchiseSoft, border: `0.5px solid ${C.franchiseSoft}`, color: C.franchise, borderRadius: 10, padding: "9px 14px", fontSize: 12.5, fontWeight: 500 }}><i className="ti ti-shield-lock" style={{ fontSize: 15 }} aria-hidden="true" />KVKK · salt-okunur</div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "0.9fr 1.4fr", gap: 18 }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 12 }}>Franchise'lar</div>
              {franchises.length === 0 ? (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "30px 0", color: C.textHint }}><i className="ti ti-building-store" style={{ fontSize: 22 }} aria-hidden="true" /><span style={{ fontSize: 12.5 }}>Bağlı franchise yok.</span></div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {franchises.map((f) => {
                    const isSel = selFran?.id === f.id;
                    const cnt = franCounts[f.id] ?? 0;
                    return (
                      <div key={f.id} onClick={() => selectFran(f)} style={{ background: isSel ? C.bg : C.surface, border: isSel ? `1.5px solid ${C.franchise}` : `1px solid ${C.border}`, borderRadius: 11, padding: "12px 14px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                          <span style={{ fontSize: 13, fontWeight: isSel ? 600 : 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{f.name}</span>
                          <span style={{ fontSize: 9, color: C.franchise, background: C.franchiseSoft, padding: "2px 7px", borderRadius: 8, fontWeight: 600, whiteSpace: "nowrap", flexShrink: 0 }}>Franchise</span>
                        </div>
                        <span style={{ fontSize: 12, color: isSel ? C.franchise : C.textFaint, fontWeight: isSel ? 600 : 400, flexShrink: 0, marginLeft: 6 }}>{cnt}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <div>
              {!selFran ? (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, padding: "60px 0", color: C.textHint, border: `1px dashed ${C.border}`, borderRadius: 14 }}><i className="ti ti-arrow-left" style={{ fontSize: 22 }} aria-hidden="true" /><span style={{ fontSize: 12.5 }}>Roster için bir franchise seçin.</span></div>
              ) : (
                <>
                  <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 12 }}>{selFran.name} — Personel ({franRoster.length})</div>
                  {franRosterLoading ? (
                    <div style={{ fontSize: 12.5, color: C.textHint, padding: "20px 0", textAlign: "center" }}>Yükleniyor…</div>
                  ) : franRoster.length === 0 ? (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "30px 0", color: C.textHint }}><i className="ti ti-user-off" style={{ fontSize: 22 }} aria-hidden="true" /><span style={{ fontSize: 12.5 }}>Personel yok.</span></div>
                  ) : (
                    <>
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {franRoster.map((em, i) => {
                          const initials = (em.first_name?.[0] || "") + (em.last_name?.[0] || "");
                          return (
                            <div key={i} style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 11, padding: "12px 14px", display: "flex", alignItems: "center", gap: 11 }}>
                              <div style={{ width: 36, height: 36, borderRadius: "50%", background: C.franchiseSoft, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 600, color: C.franchise, flexShrink: 0 }}>{initials}</div>
                              <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 600 }}>{em.first_name} {em.last_name}</div><div style={{ fontSize: 11, color: C.textFaint }}>{em.position || "—"}</div></div>
                              <span style={{ fontSize: 10, color: em.is_active ? C.greenDark : C.textHint, background: em.is_active ? C.greenSoft : C.neutralBg, padding: "3px 9px", borderRadius: 10, fontWeight: 600, flexShrink: 0 }}>{em.is_active ? "Aktif" : "Pasif"}</span>
                            </div>
                          );
                        })}
                      </div>
                      <div style={{ marginTop: 14, background: C.warnBg, border: `1px solid ${C.warnBorder}`, borderRadius: 12, padding: "11px 14px", fontSize: 11.5, color: C.warnInk, display: "flex", gap: 8 }}>
                        <i className="ti ti-shield-lock" style={{ fontSize: 15, marginTop: 1, flexShrink: 0 }} aria-hidden="true" /><span><b>KVKK koruması.</b> Franchise ayrı işverendir. Maaş, IBAN ve TC bilgisi markaya gösterilmez.</span>
                      </div>
                    </>
                  )}
                </>
      ...
      {/* ===== Modal: Personel Ekle (merkez) ===== */}
      {showForm && (
        <Modal title="Personel Ekle — Merkez" onClose={() => setShowForm(false)}>
          <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
            <div style={{ flex: 1 }}><label style={labelStyle}>Pozisyon</label><input style={inputStyle} value={form.position} onChange={(e) => setForm({ ...form, position: e.target.value })} placeholder="Operasyon Müdürü" /></div>
            <div style={{ flex: 1 }}><label style={labelStyle}>Departman</label><input style={inputStyle} value={form.department} onChange={(e) => setForm({ ...form, department: e.target.value })} placeholder="Yönetim" /></div>
          </div>
          <div style={{ marginBottom: 12 }}><label style={labelStyle}>Sözleşme Tipi</label>
            <select style={inputStyle} value={form.contract_type} onChange={(e) => setForm({ ...form, contract_type: e.target.value })}>
              <option value="full_time">Tam Zamanlı</option><option value="part_time">Yarı Zamanlı</option><option value="seasonal">Sezonluk</option><option value="contract">Sözleşmeli</option>
            </select>
          </div>
          <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
      ...
      {/* ===== Modal: Personel Detay (merkez=yonet / sube=oku) ===== */}
      {detail && (
        <Modal title="Personel Detayı" onClose={() => setDetail(null)}>
          {!editing ? (
            <div style={{ background: C.surface, borderRadius: 10, border: `1px solid ${C.border}`, padding: "14px 16px", display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
              ...
              <div><span style={{ fontSize: 11, color: C.textFaint }}>Brüt Maaş</span><div style={{ fontSize: 14 }}>{detail.base_salary != null ? `₺${Number(detail.base_salary).toLocaleString("tr-TR")}` : "—"}</div></div>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 14 }}>
              <div style={{ display: "flex", gap: 10 }}>
          ...
          {canManage ? (
            ...
            <div style={{ fontSize: 11.5, color: C.textFaint, display: "flex", alignItems: "center", gap: 6 }}><i className="ti ti-lock" style={{ fontSize: 14 }} aria-hidden="true" />Salt-okunur — şube yöneticisi yönetir.</div>
          )}
        </Modal>
      )}
    </>
